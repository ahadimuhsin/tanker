<script>
    $(document).ready(function(){
        //mendapatkan biodata peminjam berdasarkan nomor admin form anggota
        $("#no_anggota").on("change", function () {
            var no_anggota = $(this).val();
            $.ajax({
                url : "{{url('get/anggota_pinjaman_by_no_anggota/')}}" + '/'+ no_anggota,
                dataType: "json",
                type: "GET",
        ...
        $("input[name='tgl_input_pinjam']").datepicker({
            dateFormat: 'd MM yy',
            autoclose: true
        });

        $('#angsuran, #termin').on('change', function () {
            let angsuran = Math.abs($("#angsuran").val().replace(/(?:[.]|\,00)/g,''));
            let angka = document.getElementById('termin');
            // let termin = parseInt(angka);
            console.log(angka);
            console.log(angsuran);
            let total = angsuran * angka.value;

            $("#jumlah").val(total.toLocaleString('id'));
        });
        // $('#jumlah').mask('')
    });
